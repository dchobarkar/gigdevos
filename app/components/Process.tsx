    ...
    >
      <div className="mx-auto max-w-6xl">
        ...
        <div className="relative">
          ...
          <div
            className="hidden lg:block absolute top-14 left-[8.33%] right-[8.33%] h-0.5 bg-[var(--border)]"
            aria-hidden
          />

          {/* Vertical connector — mobile only */}
          <div
            className="absolute left-6 top-14 bottom-14 w-0.5 bg-[var(--border)] md:hidden"
            aria-hidden
          />
            ...
            {PROCESS_STEPS.map((step, i) => (
              ...
              >
                <div className="flex shrink-0 flex-col items-center w-8">
                  <span className="font-mono text-xs font-medium text-[var(--accent-cyan)] tabular-nums">
                    {String(i + 1).padStart(2, "0")}
                  </span>
                  <div className="glass-card rounded-full p-2.5 border border-[var(--border)] hover-glow relative z-10 mt-1">
                    <step.icon
          ...
          <div className="hidden md:grid md:grid-cols-2 lg:grid-cols-6 gap-6 lg:gap-4">
            {PROCESS_STEPS.map((step, i) => (
              ...
              >
                <span
                  className="font-mono text-xs font-medium text-[var(--accent-cyan)] tabular-nums mb-2"
                  aria-hidden
                >
                  {String(i + 1).padStart(2, "0")}
                </span>
                <div className="glass-card rounded-full p-4 border border-[var(--border)] hover-glow mb-4 relative z-10">
                  <step.icon
                    className="text-[var(--accent-cyan)]"
                    size={24}
                    strokeWidth={1.5}
